...
const Home = () => {
  ...
  const fetchPlants = async (appliedFilters = {}) => {
    try {
      ...
      const { data } = await axios.get(
        `https://urvann2.onrender.com/api/v1/plant/getPlants${query}`,
  ...
  const handleAddToCart = async (plantId) => {
    try {
      await axios.post(
        "https://urvann2.onrender.com/api/v1/cart/add",
        { plantId, quantity: 1 },
        { withCredentials: true }
      );
      // fetch updated cart
      const { data } = await axios.get("https://urvann2.onrender.com/api/v1/cart", { withCredentials: true });
      setCart({
  ...
  const handleUpdateQuantity = async (plantId, quantity) => {
    try {
      if (quantity <= 0) {
        await axios.delete("https://urvann2.onrender.com/api/v1/cart/remove", {
          data: { plantId },
          withCredentials: true,
        });
      } else {
